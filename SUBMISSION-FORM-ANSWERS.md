...
**Related:** [`SUBMISSION.md`](SUBMISSION.md) (artifact index) · [`ai-prompts/`](ai-prompts/) (full prompt log)
...
**Evidence in repo:**
- [`requirements-analysis.md`](requirements-analysis.md) — full breakdown written **before** implementation
...
- Prompt that started analysis: [`ai-prompts/` § Prompt 1](ai-prompts/_full-archive.md#prompt-1--technical-lead-planning-verbatim)
...
|-------|-----------------|----------|
| **Requirement analysis** | AI read `@Requirements`, compared Postgres vs MongoDB, mapped Core vs Stretch | [`requirements-analysis.md`](requirements-analysis.md), [Prompt 2](ai-prompts/_full-archive.md#prompt-2--stack-decision-verbatim) |
| **Planning / design** | AI drafted plans in docs; **no code until human APPROVED** | [`spec.md`](tool-specific/cursor-workflow/spec.md), [`design-notes.md`](design-notes.md), [Prompt 6](ai-prompts/_full-archive.md#prompt-6--backend-plan-verbatim-paraphrased-key-parts) |
| **Code generation** | Layer-by-layer: schema → repos → services → routes → UI | `database/`, `backend/`, `frontend/` |
| **Validation** | `npm test`, `npm run build`, cURL, manual UI walkthrough | [`test-strategy.md`](test-strategy.md), [`pr-description.md`](pr-description.md) |
| **Debugging** | Pasted terminal errors; AI diagnosed; human verified fix | [`debugging-notes.md`](debugging-notes.md), [Iteration A–E](ai-prompts/_full-archive.md#iteration-log-ai-mistakes--human-corrections) |
| **Code review** | `/code-refactorer` + `/context-aware-bug-hunter` commands; `.cursor/rules/` | [Prompt 12–13](ai-prompts/_full-archive.md#session-4--submission-artifacts--screening) |
...
- **Decision doc:** [`requirements-analysis.md` §7](requirements-analysis.md#7-database-choice-ai-recommendation--human-approval)
...
- **Fix doc:** [`debugging-notes.md` §1](debugging-notes.md#1-critical--status-update-toctou-race-backend--fixed)
...
**Evidence:** [`test-strategy.md`](test-strategy.md)

---

## Q5. Describe your debugging approach. Give an example where AI was wrong and you corrected it.

**Approach:** Paste terminal output → AI diagnoses → apply smallest fix → document in `debugging-notes.md` → re-run tests.
...
| **Evidence** | [`debugging-notes.md`](debugging-notes.md), [Iteration A](ai-prompts/_full-archive.md#iteration-a--database-port-conflict) |
...
| **Evidence** | [Iteration C](ai-prompts/_full-archive.md#iteration-c--shell-comment-paste), [`debugging-notes.md`](debugging-notes.md) |
...
| **Evidence** | [Prompt 13](ai-prompts/_full-archive.md#prompt-13--fix-all-5-issues-verbatim), [`debugging-notes.md`](debugging-notes.md) |

**Commit / file pointers for AI mistake fixes:**
- Main implementation: commit `bad0e89` (`Support Ticket Management System (Core)`)
- Post-review bug fixes: see current `backend/src/repositories/ticketRepository.ts`, `frontend/src/components/comments/CommentForm.tsx`, `frontend/src/hooks/useTicket.ts` (documented in [`debugging-notes.md`](debugging-notes.md); commit after re-submission)
...
| Human APPROVED before backend code | [Prompt 6 → 7](ai-prompts/_full-archive.md#prompt-7--approved-verbatim) |
| Human APPROVED before frontend code | [Prompt 10 → 11](ai-prompts/_full-archive.md#prompt-11--approved-verbatim) |
...
Persistent docs updated before each step: `spec.md`, `tasks.md`, `project-context.md`, `design-notes.md`.
...
**Evidence:** [`reflection.md`](reflection.md), [`ai-prompts/` § Iteration log](ai-prompts/_full-archive.md#iteration-log-ai-mistakes--human-corrections)
...
"I debugged with AI"          → debugging-notes.md + iteration log
"I chose PostgreSQL"          → requirements-analysis.md §7
"I understand trade-offs"     → reflection.md
"Prompt history exists"       → ai-prompts/ (455 lines)
```
